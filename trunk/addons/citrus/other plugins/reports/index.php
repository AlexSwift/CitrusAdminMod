<html>
	<body background = "images/background.gif">
		...
		<?php
			...
			function drawRow($color, $name, $steamID, $text, $date, $time) {
			...
			if (isset($results)) {
				...
				if ($results) {
					while ($result = mysql_fetch_array($results)) {
						...
						// Check to see if the status of the report is closed or resolved.
						if ($result['_Status'] == "Closed") {
							$color = "#DD3939";
						}elseif ($result['_Status'] == "Resolved") {
							$color = "#77DD39";
						};
						
						// Draw the row onto the table.
						drawRow($color, $result['_Name'], $result['_SteamID'], $result['_Text'], $result['_Date'], $result['_Time']);
