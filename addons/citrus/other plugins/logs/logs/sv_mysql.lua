...
local PLUGIN = citrus.Plugins.Get("Logs")
...
-- Query.
function PLUGIN.MySQL.Query(String, Callback, NoThread)
	...
	-- Check No Thread.
	if (NoThread) then
		gdatabase.Query(String, PLUGIN.MySQL.Connection)
	else
...
function PLUGIN.MySQL.Insert(Table)
	...
	-- Query.
	PLUGIN.MySQL.Query("INSERT INTO "..PLUGIN.Configuration["Table"].." ("..Arguments..") VALUES("..Values..")", nil, true)
